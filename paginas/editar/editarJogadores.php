...
$idj = filter_input(INPUT_GET, 'idJogador');
if ($idj) {
    $sql = "SELECT * 
            FROM jogadores   
            WHERE idJogador = $idj";
...
$sql1 = "SELECT * FROM localizacao ORDER BY localizacao.nome ASC";
$resultado = $conn->query($sql1);


$sql_jogador = "SELECT * FROM jogadores WHERE idJogador = $idj";
...
if (isset($_POST['submit'])) {
    ...
    $SQLdados = "UPDATE jogadores SET nome = '$nome', idade = '$idade',idLocalizacao = '$localizacao', posicao = '$posicao',numCamisa = '$camisa' WHERE idJogador = $idj";
...
if (isset($_POST['editarImagem'])) {
    ...
    $sql_foto = "UPDATE jogadores SET fotoPerfil = '$novo_nome' WHERE idJogador = $idj";
...
if (isset($_POST['submita'])) {
    $sql_delete = "DELETE FROM contratojogadores WHERE idJogador = $idj";
    $res_delete = $conn->query($sql_delete);
    $res_delete->execute();

    $sql_delJogador = "DELETE FROM jogadores WHERE idJogador = $idj";
...
<head>
    ...
    <title>Editar Jogador | TwoFootball</title>
    <link rel="stylesheet" href="../../styles/sidebar.css">
    <link rel="stylesheet" href="styles/editarJogadores.css">
    <link rel="stylesheet"
...
<body>
    ...
    <div>
        <form action="" method="post" id="formDados">
            ...
            <img src="../../imagens/logo.png" width="400px" style="margin: 10px 0px -10px 45px;">

            <label for="nome">
                <p class="paragrafo">Nome:<strong>*</strong></p>
            </label>
            <input class="cxdt" type="text" value="<?= $reg_update['nome'] ?>" name="nome" required />

            <label for="idade">
                <p class="paragrafo">Idade:<strong>*</strong></p>
            </label>
            <input class="cxdt" type="number" value="<?= $reg_update['idade'] ?>" name="idade" required />

            <label onclick="abrirMudarImagem()" id="abrirTelaImagem" style="cursor: pointer;">Clique aqui para alterar a foto de
                perfil<strong>*</strong></label>
            ...
            <select name="localizacao">
                ...
                <?php foreach ($resultado as $reg): ?>
                    <option value="<?= $reg['idLocalizacao']; ?>">
                        <?= $reg['nome']; ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <input class="cxdt" type="text" value="<?= $reg_update['posicao'] ?>" name="posicao" required />

            <label for="numCamisa">
                <p class="paragrafo">Número da Camisa:<strong>*</strong></p>
            </label>
            <input class="cxdt" type="number" value="<?= $reg_update['numCamisa'] ?>" name="numCamisa" required />
            <?php
            if ($login) {
                echo ' <input id="adicionar" type="submit" value="EDITAR JOGADOR" name="submit" style="margin-top: 5px;">
                <input id="adicionar" type="submit" value="APAGAR JOGADOR" name="submita" style="margin-top: 5px;">';
            } else {
                echo ' <input id="adicionar" type="submit" value="EDITAR JOGADOR" name="submit" style="margin-top: 5px;" disabled>
                <input id="adicionar" type="submit" value="APAGAR JOGADOR" name="submita" style="margin-top: 5px;" disabled>';
            }
            ?>
        </form>
    </div>
    ...
    <div>
        <form action="" method="post" enctype="multipart/form-data" id="mudarImagem">
            <img src="../../imagens/logo.png" width="300px" style="margin: -5px 0px 0px 85px;">
            <img src="../../usuario/uploads/x.png" width="30px" style="margin: -5px 0px 0px 60px; cursor: pointer;"
                onclick="fecharMudarImagem()">
            <label for="fotoPerfil">Foto de Perfil:<strong>*</strong></label>
            <input id="fotoPerfil" class="form-control" type="file"
                style="margin-top: 5px; margin-left: 13px; margin-bottom: 15px;" name="fotoPerfil"
                style="font-size: 15px; width: 320px;" accept=".jpg,.jpeg,.png">
            <?php
            if ($login) {
                echo '<input id="addImagem" type="submit" value="EDITAR IMAGEM" name="editarImagem">';
            } else {
                echo '<input id="addImagem" type="submit" value="EDITAR IMAGEM" name="editarImagem" disabled>';
            }
            ?>
        </form>
    </div>
    ...
    <script>
        function abrirMudarImagem() {
            document.getElementById('mudarImagem').style.display = "block";
        }
        function fecharMudarImagem() {
            document.getElementById('mudarImagem').style.display = "none";
        }
    </script>
