...
$idj = filter_input(INPUT_GET, 'idClube');
if ($idj) {
    $sql = "SELECT * 
            FROM clubes   
            WHERE idClube = $idj";
...
$sql_jogador = "SELECT * FROM clubes WHERE idClube = $idj";
...
if (isset($_POST['submit'])) {
    ...
    $SQLdados = "UPDATE clubes SET nome = '$nome', dataFundacao = '$dataFundacao',idLocalizacao = '$localizacao' WHERE idClube = $idj";
...
if (isset($_POST['editarImagem'])) {
    ...
    $diretorio = "../../imagens/FotosClube/";
    move_uploaded_file($_FILES['fotoPerfil']['tmp_name'], $diretorio . $novo_nome);

    $sql_foto = "UPDATE clubes SET logo = '$novo_nome' WHERE idClube = $idj";
...
if (isset($_POST['submita'])) {
    $sql_delete = "DELETE FROM contratojogadores WHERE idClube = $idj";
    $res_delete = $conn->query($sql_delete);
    $res_delete->execute();

    $sql_delJogador = "DELETE FROM clubes WHERE idClube = $idj";
...
<head>
    ...
    <title>Editar Clubes | TwoFootball</title>
    <link rel="stylesheet" href="styles/editarClube.css">
...
<nav style="margin-top: 0px;" id="menu" class="menu" onmouseenter="abrir()" onmouseleave="abrir()">
    ...
    <div id="formulario">
        <form action="" method="post">
            ...
            <input class="cxdt" type="text" name="nome" value="<?= $reg_update['nome'] ?>" placeholder="Ex: Champions League" required />

            <label for="dataFundacao">
                    <p class="paragrafo">Data de Fundação:<strong>*</strong></p>
                </label>
                <input type="date" value="<?= $reg_update['dataFundacao'] ?>" name="dataFundacao">
            ...
            <?php
            if ($login) {
                echo ' <input id="adicionar" type="submit" value="EDITAR CLUBE" name="submit" style="margin-top: 5px;">
                <input id="adicionar" type="submit" value="APAGAR CLUBE" name="submita" style="margin-top: 5px;">';
            } else {
                echo ' <input id="adicionar" type="submit" value="EDITAR CLUBE" name="submit" style="margin-top: 5px;" disabled>
                <input id="adicionar" type="submit" value="APAGAR CLUBE" name="submita" style="margin-top: 5px;" disabled>';
            }
            ?>
    ...
    <div id="editarFoto">
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <img src="../../usuario/uploads/x.png" style="margin-left: 40px; margin-bottom: -5px; cursor: pointer; width: 30px; height: 30px;" onclick="fecharEditarFoto()">
